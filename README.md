...
Every response from `/chat` includes a `Content-Security-Policy` header with a per-request
nonce generated by `crypto/rand`. The inline `<script>` block in the chat page requires that
nonce to execute. The CDN scripts (Tailwind, marked, DOMPurify) are explicitly allowlisted by
host. This prevents injected scripts from running even if an XSS vector were found elsewhere.

`style-src` includes `'unsafe-inline'` intentionally — the Tailwind Play CDN injects `<style>`
tags at runtime and cannot be nonced. This is a known limitation of the CDN delivery approach.

### Why CSRF protection is not needed

Cross-Site Request Forgery (CSRF) attacks work by tricking a logged-in user's browser into
making an authenticated request to your server. The attack relies on the browser automatically
attaching credentials (cookies, HTTP Basic auth) to cross-origin requests.

This application uses **Bearer token authentication**: the client reads the key from
`localStorage` and explicitly sets an `Authorization: Bearer <key>` header on every API
request. Browsers do not automatically attach custom headers cross-origin — a malicious
third-party page cannot read from `localStorage` or set the `Authorization` header on a
request to this server. CSRF attacks targeting the `/api/v1/*` routes are therefore not
possible regardless of origin.

The `nosurf` CSRF middleware was evaluated and intentionally skipped because it would guard
zero routes in this architecture — adding it would produce ceremony with no security benefit.
